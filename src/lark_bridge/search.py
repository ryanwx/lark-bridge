...
async def search_msg_ids(
    ...
    while len(all_msg_ids) < limit:
        page = await search_msg_ids_page(
            cookie, chat_id, start_time, end_time, from_id, mention_user_id, page_token, domain
        )
        if not page["msg_ids"]:
            break
        all_msg_ids.extend(page["msg_ids"])
...
async def search_msg_ids_page(
    ...
    Returns dict: {"msg_ids": [...], "has_more": bool, "page_token": str}
    ...
    page_token_bytes = page_token.encode() if page_token else b""
    ...
    rid = hashlib.md5(f"{time.time()}page".encode()).hexdigest()[:10]
    pkt = pb.Packet()
    pkt.payloadType = 1
    pkt.cmd = 11021
    pkt.payload = inner
    ...
    if json_str:
        try:
            ...
            if has_more:
                next_page_token = json.dumps(j, separators=(",", ":"))
        except Exception:
            pass
